" vim-plastic theme is a variant of Ramzi Akremi's vim-one theme. All
" vimscript in this file originates from that theme.
...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  fun <sid>X(group, fg, bg, attr)
    let l:attr = a:attr
    if g:plastic_allow_italics == 0 && l:attr ==? 'italic'
      let l:attr= 'none'
    endif
  ...
  if &background ==# 'dark'
    let s:dark = 1
    let s:mono_1 = 'abb2bf' " Cadet Blue
    let s:mono_2 = '737c8c' " Raven
    let s:mono_3 = '5c6370' " Shuttle Gray
    let s:mono_4 = '3E4451' " Bright Gray
    let s:mono_5 = 'd2d6db' " Iron

    let s:hue_1  = '56b6c2' " Fountain Blue
    let s:hue_2  = '61afef' " Cornflower Blue
    let s:hue_3  = 'c678dd' " purple
    let s:hue_4  = '98c379' " Olivine

    let s:hue_5   = 'e06c75' " Sunglo
    let s:hue_5_2 = 'ff0000' " Sunglo

    let s:hue_6   = 'd19a66' " Whiskey
    let s:hue_6_2 = 'e5c07b' " Harvest Gold

    let s:syntax_bg     = '21252b' " Shark
    let s:syntax_gutter = 'ff0000'
    let s:syntax_cursor = '3E4451'

    let s:syntax_accent = s:hue_5

    let s:vertsplit    = '2e343c' " Bunker
    "let s:vertsplit    = '080a0f' " Bunker
    let s:special_grey = '3e4451'
    let s:visual_grey  = '3e4451'
    let s:pmenu        = '181A1F' " Woodsmoke

" TODO Things to fix
" Rust or Python use of Include
  ...
  call <sid>X('Constant',       s:hue_2,         '',          '')
  call <sid>X('String',         s:hue_4,         '',          '')
  call <sid>X('Character',      s:hue_2,         '',          '')
  call <sid>X('Number',         s:hue_1,         '',          '')
  call <sid>X('Boolean',        s:hue_1,         '',          '')
  call <sid>X('Float',          s:hue_1,         '',          '')
  call <sid>X('Identifier',     s:mono_5,        '',          'bold')
  call <sid>X('Function',       s:hue_6,         '',          '')
  call <sid>X('Statement',      s:hue_5,         '',          'none')
  call <sid>X('Conditional',    s:hue_5,         '',          '')
  call <sid>X('Repeat',         s:hue_5,         '',          '')
  call <sid>X('Label',          s:hue_3,         '',          '')
  call <sid>X('Operator',       s:syntax_accent, '',          'none')
  call <sid>X('Keyword',        s:hue_2,         '',          '')
  call <sid>X('Exception',      s:hue_5,         '',          '')
  call <sid>X('PreProc',        s:hue_6_2,       '',          '')
  call <sid>X('Include',        s:hue_5,         '',          '')
  call <sid>X('Define',         s:hue_2,         '',          'none')
  call <sid>X('Macro',          s:hue_6,         '',          '')
  call <sid>X('PreCondit',      s:hue_6_2,       '',          '')
  call <sid>X('Type',           s:hue_6,         '',          'none')
...
"}}}
" Public API --------------------------------------------------------------{{{
function! plastic#highlight(group, fg, bg, attr)
